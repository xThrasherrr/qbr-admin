...
-------- COMMANDS --------
--------------------------
exports['qbr-core']:AddCommand('staffchat', "Send Message to Staff", {{name='message', help='Message'}}, true, function(source, args)
  local msg = table.concat(args, ' ')
  TriggerClientEvent('qb-admin:client:SendStaffChat', -1, GetPlayerName(source), msg)
end, 'admin')

exports['qbr-core']:AddCommand('announce', "Make Announcement", {}, false, function(_, args)
  local msg = table.concat(args, ' ')
...
exports['qbr-core']:AddCommand('warn', "Warn a Player", {{name='ID', help='Player'}, {name='Reason', help='Mention a reason'}}, true, function(source, args)
  local targetPlayer = exports['qbr-core']:GetPlayer(tonumber(args[1]))
  local senderPlayer = exports['qbr-core']:GetPlayer(source)
  table.remove(args, 1)
  local msg = table.concat(args, ' ')
  ...
  TriggerClientEvent('chat:addMessage', source, { args = { "SYSTEM", "[WARNED]: "..GetPlayerName(targetPlayer.PlayerData.source).." [REASON]: "..msg }, color = 255, 0, 0 })
    ...
    TriggerClientEvent('QBCore:Notify', src, 8, 'Not Online', 5000, 'Player is not online!', 'mp_lobby_textures', 'cross', 'COLOR_RED')
  end
end, 'admin')

exports['qbr-core']:AddCommand('admin', 'Open the admin menu (Admin Only)', {}, false, function(source)
  local src = source
  TriggerClientEvent('qbr-admin:OpenMainPage', src)
end, 'admin')
...
local savedCoords = {}
exports['qbr-core']:AddCommand('bring', 'Bring a player to you (Admin only)', { { name = 'id', help = 'Player ID' }, }, true, function(source, args)
  if args[1] then
    local src = source
...
exports['qbr-core']:AddCommand('bringback', 'Bring back a player (Admin only)', { { name = 'id', help = 'Player ID' }, }, true, function(source, args)
  if args[1] then
    local src = source
...
exports['qbr-core']:AddCommand('goto', 'Teleport yourself to the player (Admin only)', { { name = 'id', help = 'Player ID' }, }, true, function(source, args)
  if args[1] then
    local src = source
    local admin = GetPlayerPed(src)
...
----- REPORT COMMANDS -----
----------------------------
exports['qbr-core']:AddCommand('report', "Send Admin Report", {{name='message', help='Message'}}, true, function(source, args)
  local src = source
  local msg = table.concat(args, ' ')
...
exports['qbr-core']:AddCommand('reportr', "Respond to Report", {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args)
  local src = source
  ...
  if msg == '' then return end
  if not OtherPlayer then return TriggerClientEvent('QBCore:Notify', src, 8, 'Not Online', 5000, 'Player is not online!', 'mp_lobby_textures', 'cross', 'COLOR_RED') end
  if not exports['qbr-core']:HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') ~= 1 then return end
  ...
  TriggerClientEvent('QBCore:Notify', src, 8, 'Reply Sent', 5000, 'Report reply sent', 'mp_lobby_textures', 'cross', 'COLOR_RED')
...
------ DEVELOPER COMMANDS --------
----------------------------------
exports['qbr-core']:AddCommand('vector3', 'Copy vector3 to clipboard (Admin only)', {}, false, function(source)
  local src = source
  TriggerClientEvent('qb-admin:client:copyToClipboard', src, 'coords3')
end, 'admin')

exports['qbr-core']:AddCommand('vector4', 'Copy vector4 to clipboard (Admin only)', {}, false, function(source)
  local src = source
  TriggerClientEvent('qb-admin:client:copyToClipboard', src, 'coords4')
end, 'admin')

exports['qbr-core']:AddCommand('heading', 'Copy heading to clipboard (Admin only)', {}, false, function(source)
  local src = source
...
RegisterNetEvent('qb-admin:server:Staffchat:addMessage', function(name, msg)
  local src = source
  if exports['qbr-core']:HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
    if exports['qbr-core']:IsOptin(src) then
      TriggerClientEvent('chat:addMessage', src, {
        color = {255, 0, 0},
        multiline = true,
...
RegisterNetEvent('admin:server:kick', function(player, reason)
  ...
  if exports['qbr-core']:HasPermission(src, permissions['kick']) or IsPlayerAceAllowed(src, 'command') then
    ...
    DropPlayer(player.id, "You have been kicked from the server" .. ':\n' .. reason .. '\n\n' .. "🔸 Check our Discord for more information: " .. exports['qbr-core']:GetConfig().Server.discord)
  end
end)
